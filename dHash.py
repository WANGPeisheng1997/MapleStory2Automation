class DHash(object):
    @staticmethod
    def calculate_hash(image, name=None):
        """
        计算图片的dHash值
        :param image: PIL.Image
        :return: dHash值,string类型
        """
        difference = DHash.__difference(image, name)
    ...
    @staticmethod
    def __difference(image, name=None):
        """
        *Private method*
        计算image的像素差值
        :param image: PIL.Image
        ...
        smaller_image.save(name + "s.png")
        # 2. 灰度化 Grayscale
        grayscale_image = smaller_image.convert("L")
        grayscale_image.save(name + "g.png")
        # 3. 比较相邻像素
